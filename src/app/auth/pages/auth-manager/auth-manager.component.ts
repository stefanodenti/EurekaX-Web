...
export class AuthManagerComponent {
  ...
  constructor(private queryService: QueryFirestoreService, private notificationService: NotificationsService) {
    this.search('auth-actions');
  }

  search(collection: string){
    this.queryService.search(this.filters, 'name', 10, null, collection).subscribe({
      next: (res) => {
        console.log('ReS', res);
